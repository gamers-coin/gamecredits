...
def check_json_precision():
    """Make sure json library being used does not lose precision converting GAME values"""
...
def read_gamecredits_config(dbdir):
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "gamecredits.conf"))))
...
def create_tx(gamecreditsd, fromaddresses, toaddress, amount, fee):
    ...
    if total_available < needed:
        sys.stderr.write("Error, only %f GAME available, need %f\n"%(total_available, needed));
        sys.exit(1)
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of gamecredits.conf file with RPC username/password (default: %default)")
