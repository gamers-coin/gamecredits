...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a GameCredits address (e.g. %1)").arg("Gq3Gyigyd12kJDkhwi9M9QSZ9qu6M4NZzR"));
#endif
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::GAME, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::GAME, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
boost::filesystem::path static StartupShortcutPath()
{
    if (GetBoolArg("-testnet", false))
        ...
        return GetSpecialFolderPath(CSIDL_STARTUP) / "GameCredits (regtest).lnk";
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (GetBoolArg("-testnet", false))
            optionFile << "Name=GameCredits (testnet)\n";
        else if (GetBoolArg("-regtest", false))
            optionFile << "Name=GameCredits (regtest)\n";
        else
