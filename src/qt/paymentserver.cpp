...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the gamecredits: URI contains a payment request, we are not able to detect the
...
//
// OSX-specific way of handling gamecredits: URIs and PaymentRequest mime types.
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom gamecredits addresses are not supported
        ...
        // GameCredits amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
