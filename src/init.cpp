...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("GameCredits version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  gamecreditsd [options]                     " + "\n" +
                  "  gamecreditsd [options] <command> [params]  " + _("Send command to -server or gamecreditsd") + "\n" +
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: gamecredits.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: gamecreditsd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GameCredits is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("GameCredits version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
